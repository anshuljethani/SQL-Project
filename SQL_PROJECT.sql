...
-- 1. Problem statement - show the list of employees by department
-- Question: Write a query to list all employees along with their respective department
-- names. Include employee number, first name, last name, department number, and
-- department name.

-- OBSERVATION :- in employees table we have pk as emp_no, in dept_emp we have emp_no as fk and dept_no as fk , in departments we have dept_no as pk
-- so we can relate the three and list the list of employees by department
...
select max(salary) as salary , emp_no , from_date, to_date from salaries
where emp_no = '10044' 
...
-- Question: Write a queryto count the number of each departments, include department number, department name and employee count.
